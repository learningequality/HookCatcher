...
def get_ngrok(local_url):
    if "localhost:8000" in local_url:
        return local_url.replace("localhost:8000", "6f884b03.ngrok.io")

    elif "127.0.0.1:8000" in local_url:
        return local_url.replace("127.0.0.1:8000", "6f884b03.ngrok.io")
...
def browserstack(state_obj, config):
    ...
    if config['browser_version']:
        BROWSER = config['browser'] + config['browser_version']      # might need [0] even if only one item
    else: 
    ...
    # generate new image if no image like this is in data or if there is 1 but the file doesn't exist
    if img_query.count() == 0 or img_query.count() == 1 and not img_exists(img_query.get().img_file.name):
        with tempfile.NamedTemporaryFile(suffix='.png') as temp_img:
            ...
            # avaliable options for deices here --> https://www.browserstack.com/screenshots/browsers.json
            data = {"url": get_ngrok(state_obj.state_url), 
                    "callback_url": "http://ba3c69ba.ngrok.io/bs_callback/{0}".format(img_obj.id),
                    "win_res": '{0}x{1}'.format(config['desktop_resolution'][0], config['desktop_resolution'][1]), 
                    "mac_res": '{0}x{1}'.format(config['desktop_resolution'][0], config['desktop_resolution'][1]), 
                    "quality": "compressed", 
                    "wait_time": 5, 
                    "local": "true",
            ...
            bs_request = requests.post('https://www.browserstack.com/screenshots', 
                                       ...
                                       auth=(settings.BROWSERSTACK_USERNAME, settings.BROWSERSTACK_OAUTH))
            if bs_request.status_code == 200:
                bs_job_url = 'https://www.browserstack.com/screenshots/{0}.json'.format(json.loads(bs_request.text)['job_id'])
                print('Browserstack is currently generating the image. Check on the progress: {0}'.format(bs_job_url))
                return img_obj
            else:
                img_obj.delete()
